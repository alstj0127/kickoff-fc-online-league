            ...
            return (
              <article className="league-list-item" key={item.id}>
                ...
                <Link
                  ...
                  href={`/leagues/${encodeURIComponent(item.id)}`}
...
function LeagueView({
  ...
  useEffect(() => {
    const pinTimer = window.setTimeout(() => {
      setEditPin(sessionStorage.getItem(`kickoff-pin-${league.id}`) ?? "");
    }, 0);
...
export function SetupPage({ notice }: { notice?: string }) {
  const router = useRouter();

  function created(next: League, pin: string) {
    sessionStorage.setItem(`kickoff-pin-${next.id}`, pin);
    router.push(`/leagues/${encodeURIComponent(next.id)}`);
  }

  return <SetupView onCreated={created} notice={notice} />;
}
...
export function LeaguePage({ leagueId }: { leagueId: string }) {
  ...
  if (league)
    return <LeagueView league={league} onLeagueChange={setLeague} />;
  return null;
}
